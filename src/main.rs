...
thread_local! {
    ...
    // `$HOME/.cache/cargo-unmaintained`.
    static ON_DISK_REPOSITORY_CACHE_ONCE_CELL: RefCell<OnceCell<repository_cache::Cache>> = RefCell::new(OnceCell::new());
}
                    ...
                    || {
                        ...
                        match url_and_dir {
                            Ok((url_string, repo_dir)) => {
                                ...
                                // acting as a key in a global map, so this is okayish.
...
fn on_disk_repository_cache(
    ...
    let _: &repository_cache::Cache = once_cell.get_or_init(|| {
        ...
        .unwrap_or_else(|error| panic!("failed to create repository cache: {error}"))
    });
